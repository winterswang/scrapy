...
import sys
import re
class soufun_database_spider(CrawlSpider):
	name = "soufunG2"
	allowed_domains = ["soufun.com"]
	start_urls = []
	rules = [
		Rule(SgmlLinkExtractor(allow=('/agent/agentnew/aloneesfhlist.aspx.*','/Agent/AgentNew/AloneEsfHList.aspx.*')), follow=True),
	...
	def getData(self):
		try:
			...
			cur=conn.cursor()
			cur.execute('select url from house_soufun limit 0,10')
			result=cur.fetchall()
			url = []
			for u in result:
				url.append(u[0])
