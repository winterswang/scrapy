...
class SQLHousePipeline(object):
    ...
    def _conditional_insert(self, tx, item):
        ...
        data = [item['houseid'],item['title'],item['description'],item['agent_tel'], item['url'],item['types'],item['labels'],item['location'],now,item['store_url']]
        result = tx.fetchone()
        if result:
            data.append(item['houseid'])
            tx.execute(
                "update house_soufun set house_id=%s,title=%s, description=%s, agent_tel=%s,url=%s,house_type=%s,labels=%s,location=%s,createtime=%s,store_url=%s where house_id=%s" ,data
            ) 
            self.update_agent_store_updatecount(tx,item['store_url'])
            log.msg("Item updated in db: %s" % item, level=log.DEBUG)
        else:
            tx.execute(
                "insert into house_soufun (house_id, title, description, agent_tel,url,house_type,labels,location,createtime,store_url) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)" , data
            )
