	...
	def getData(self):
		try:
			...
			if result:
				...
				cur.execute("update agent_store set check_status =%s,update_count =0,insert_count=0,is_finish = 2 where store_url=%s and agent_open_id=%s",(check_status,result[0],result[2]))
				conn.commit()
			cur.close()
			conn.close()
			return result
